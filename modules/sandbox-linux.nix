...
let
  defaultPackageNames = [ "coreutils" "bash" "python3" "iputils" ];
  networkRequiredPaths = [ "/etc/resolv.conf" "/etc/ssl/certs" "/etc/static/ssl" ];
  ...
  readOnlyPaths = (filesystem.readOnly or []) ++ lib.optionals networkEnabled networkRequiredPaths;
